...
class VGG16FCN:
    ...
    def __init__(self, vgg16_npy_path=None):
        if not os.path.isfile(vgg16_npy_path):
            logging.error(("File '%s' not found. Download it from ftp://mi.eng.cam.ac.uk/pub/mttt2/models/vgg16.npy"), vgg16_npy_path)
            sys.exit(1)
        
        logging.info(".....Loading pre-trained weights from '%s'.", vgg16_npy_path)
        self.data_dict = np.load(vgg16_npy_path, encoding='latin1').item()
               

    def build(self, rgb, train=False, num_classes=None, random_init_fc8=False):        
    ...
    def _conv_layer(self, bottom, name):
        ...
        logging.info('.......Layer ' +  name + ' ' + str(shape))
    ...
    def _fc_layer(self, bottom, name, num_classes=None, relu=True):
        shape = self.data_dict[name][0].shape
        logging.info('.......Layer ' +  name + ' ' + str(shape))
    ...
    def _score_layer(self, bottom, name, num_classes):
        with tf.variable_scope(name) as scope:           
            ...
            w_decay = self.wd
            weights = self._variable_with_weight_decay(shape, stddev, w_decay, decoder=True)

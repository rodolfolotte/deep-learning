...
def run_training(hypes, modules, tv_graph, tv_sess, start_step=0):
    ...
    for step in xrange(start_step, hypes['solver']['max_steps']):
        ...
        # Do a evaluation and print the current state. Write checkpoint to disk
        if step % eval_iter == 0 and step > 0 or (step + 1) == hypes['solver']['max_steps']:            
            logging.info('..Running Evaluation Script')
            eval_dict, images = modules['eval'].evaluate(hypes, sess, tv_graph['image_pl'], tv_graph['inf_out'])

            logging.info("..Evaluation Finished. Saving images in (hypes[dirs][output_dir]):")
        ...
        if step % save_iter == 0 and step > 0 or (step + 1) == hypes['solver']['max_steps']:
            ...
            logging.info('..Writing checkpoint: ' + hypes['dirs']['output_dir'])
...
def continue_training(logdir):
    ...
    with tf.Session() as sess:
        ...
        logging.info("..Continue training. Loading weigths")
        cur_step = core.load_weights(logdir, sess, saver)
        if cur_step is None:
            logging.warning("..Loaded global_step is None.")
            logging.warning("..This could mean that no weights have been loaded.")

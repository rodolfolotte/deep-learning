...
def do_analyze(logdir, base_path=None):
    ...
    This will load model files and weights found in logdir and run a basic analysis.
    ...
    with tf.Graph().as_default():
        ...
        logging.info("Output Images will be written to: {}".format(os.path.join(output_dir, "images/")))

        logging_file = os.path.join(logdir, "analyse/output.log")
        utils.create_filewrite_handler(logging_file)

        eval_dict, images = modules['eval'].evaluate(hypes, sess, image_pl, inf_out)

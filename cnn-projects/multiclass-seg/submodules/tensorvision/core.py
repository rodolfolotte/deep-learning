...
tf.app.flags.DEFINE_boolean('summary', True, ('Whether or not to save summaries to tensorboard.'))
...
def build_training_graph(hypes, queue, modules):    
    ...
    logging.info("...Run inference on the encoder network")
    logits = encoder.inference(hypes, image, train=True)

    logging.info("...Build decoder on top of the logits")
    decoded_logits = objective.decoder(hypes, logits, train=True)

    logging.info("...Add to the Graph the Ops for loss calculation.")
    with tf.name_scope("Loss"):
        losses = objective.loss(hypes, decoded_logits, labels)

    logging.info("...Add to the Graph the Ops that calculate and apply gradients.")
    ...
    logging.info("...Evaluating training graph")
